...
dont l'Etat finance ses d\'{e}penses n'a pas d'effet réel. Il est équivalent que l'\'Etat se finance par impôt ou par dette. L'argument est le suivant. Si l'\'Etat se finance par émission de dette, les agents anticipent parfaitement l'augmentation de taxes futures et ce faisant diminuent leur consommation de la même façon.   \bigskip
Les hypothèses sous--jacentes à la validité de l'équivalence ricardienne sont les suivantes~:
\begin{enumerate}[(i)~]
	\item Horizon de vie infini : sinon ce sont d'autres agents qui rembourseront la dette\ldots
	\item Taxes {\it lump--sum} : sinon les taxes créent des distorsions sur le comportement des agents et cela implique que le timing des taxes comptent.
	\item Pas d'environnment stochastique : sinon l'anticipation des agents diffère de la réalisation.
\end{enumerate}
...
dynastique. Les agents qui ont contracté la dette doivent se sentir `responsables' de son remboursement. 
\bigskip

Dans ce modèle, les individus ne sont pas altruistes et ont une probabilité de cdécès exogène, indépendante de leur âge. 
...
\correction{
La taille de la population à la date $t+1$ s'écrit~:
\begin{equation*}
...
Le taux de croissance de la population est positif s'il y a plus de naissances que de décès~!
...
On suppose que la loi des grands nombres s'applique de sorte
...
\begin{itemize}
	...
	\item via une prime proportionnelle ($\equiv$ rendement viager lié aux décès des autres) \`{a} son \'{e}pargne.
\end{itemize}
...
\correction{
L'agent place son épargne $a_{s,t}+w_{t}-\tau _{t}-c_{t}$ qui lui raporte le taux sans risque et un rendement 
...
nul.\medskip
\correction{
On suppose que le syst\`{e}me viager est concurrentiel (hypothèse de libre entrée). Le compte de résultat du système viager est~:
\begin{itemize}
\item d\'{e}penses en p\'{e}riode $t$ : 
\begin{equation*}
\sum_{s<t}(1-p)^{t-s}\Pi (a_{s,t-1}+w_{t-1}-\tau _{t-1}-c_{t-1})bN_{s}
\end{equation*}
\item recettes en p\'{e}riode $t$ : 
\begin{equation*}
\sum_{s<t}p(1-p)^{t-1-s}(1+r)(a_{s,t-1}+w_{t-1}-\tau _{t-1}-c_{t-1})bN_{s}
\end{equation*}
\end{itemize}

La condition de profit nul implique : 
\begin{eqnarray*}
(1-p)\Pi &=&p(1+r)  \\
\textrm{et}\quad \Pi &=&(1+r)\frac{p}{1-p}
\end{eqnarray*}

Le rendement total versé aux survivants (=$(1-p)\Pi$) est égal au rendement total de des décédés ($p(1+r)$).
}
...
1+r}{1-p}$.
\correction{
La r\'{e}mun\'{e}ration totale de l'\'{e}pargne s'écrit~:
\begin{eqnarray*}
1+r_{h} &=&1+r+\Pi =1+r+(1+r)\frac{p}{1-p} \\
&=&\frac{1+r}{1-p}
\end{eqnarray*}

Le rendement sans risque est égal à l'espérance du rendement total de l'épargne~: avec proba $p$ je décède et je cède toute ma richesse (rendement 0) et avec proba $1-p$ je survis et ma richesse rapporte $1+r_h$. 
}
...
L'\'{e}volution de la richesse est contrainte par la condition suivante (condition de transversalité) : 
\begin{equation*}
\lim_{t\rightarrow \infty }\quad \frac{a_{s,t}}{(1+r_{h})^{t}}=0
\end{equation*}

\ques Ecrire le programme de maximisation d'un agent n\'{e} \`{a} la p\'{e}%
riode $s$ et le Lagrangien associ\'{e}.

\correction{Le programme est le suivant :
\begin{eqnarray*}
&& \max \quad \sum_{t=s}^{\infty }[{(1-p)\beta ]^{t-s}\log (c_{s,t})} \\
&&\textrm{s.c.}\quad a_{s,t+1}=(1+r_{h})(a_{s,t}+w_{t}-\tau _{t}-c_{t}) \\
&&\phantom{\textrm{s.c.}}\quad \lim_{t\rightarrow \infty }\quad \frac{a_{s,t}}{(1+r_{h})^{t}}=0 \\
&&\phantom{\textrm{s.c.}}\quad a_{s,s}=0
\end{eqnarray*}
...
\lambda_{s,t+1}(1+r_{h})\beta (1-p) &=& \lambda _{s,t}
\end{eqnarray*}

On en déduit alors la condition d'Euler~:
\begin{equation*}
...
Il ne reste alors qu'à la sommer pour obtenir la contrainte budgétaire intertemporelle.
\begin{equation*}
\sum_{i=0}^{\infty }\frac{c_{s,t+i}}{%
...
\ques Ecrire la loi d'\'{e}volution de la dette $D_{t}$.
\correction{
\begin{equation*}
...
\correction{
La contre-partie de la richesse financi\`{e}re agr\'{e}g\'{e}e sur tous
...
dents budg\'{e}taires futurs.

\correction{
Prise en compte de la contrainte budg\'{e}taire intertemporelle de l'Etat
...
\end{equation*}
\correction{
On fait appara\^{\i}tre dans la richesse humaine les imp\^{o}ts futurs :
...
correspondants sont identiques si et seulement si $b=0$. }
\medskip
...
\correction{
Ici, ce qui compte, ce sont les naissances futures, qui \'{e}largissent
